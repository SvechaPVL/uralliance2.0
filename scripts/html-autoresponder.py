...
def send_html_response(to_email: str, original_subject: str):
    """Отправляет HTML-ответ через SMTP."""
    try:
        # Создаём письмо
        msg = MIMEMultipart('alternative')
        msg['From'] = formataddr(('ООО ЮрАльянс', CONFIG['email']))
        msg['To'] = to_email
        msg['Subject'] = f'Re: {original_subject}' if original_subject else 'Заявка получена — ЮрАльянс'

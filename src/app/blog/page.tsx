...
export default async function BlogPage() {
  ...
  return (
    <>
      <BreadcrumbJsonLd
        items={[{ name: "Главная", url: "https://uralliance.ru" }, { name: "Блог" }]}
      />
      <Section spacing="xl">
        <Container className="space-y-16">
          <header className="mx-auto max-w-3xl space-y-6 text-center">
            ...
            <div className="space-y-4">
              ...
              <Text size="lg" tone="secondary">
                Делимся стратегиями, которые помогают клиентам выигрывать суды, запускать LegalTech
                и масштабировать продукты без хаоса.
              </Text>
            </div>
          </header>
          ...
          {posts.length === 0 ? (
            <div className="rounded-3xl border border-dashed border-[var(--color-border)] p-12 text-center">
              <Text size="lg" tone="secondary">
                Мы готовим первые материалы. Подпишитесь на Telegram, чтобы узнать о запуске
                первыми.
              </Text>
            </div>
          ) : (
